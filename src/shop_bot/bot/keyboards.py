...
def create_payment_method_keyboard(
    ...
    if payment_methods and payment_methods.get("urlpay"):
        builder.button(text="💳 СБП / URLPay", callback_data="pay_urlpay")
    ...
    builder.button(text=(get_setting("btn_back") or "⬅️ Назад"), callback_data="back_to_plans")
    builder.adjust(1)
    return builder.as_markup()
...
def create_topup_payment_method_keyboard(payment_methods: dict) -> InlineKeyboardMarkup:
    ...
    if payment_methods and payment_methods.get("urlpay"):
        builder.button(text="💳 СБП / URLPay", callback_data="topup_pay_urlpay")
    if payment_methods and payment_methods.get("yoomoney"):
...
async def create_key_info_keyboard(key_id: int) -> InlineKeyboardMarkup:
    """Create keyboard with app-specific buttons for VPN key.
    Shows URL buttons that redirect to app-specific subscription links via shortened URLs.
    """
    ...
    # Get key data to fetch subscription links
    key_data = get_key_by_id(key_id)

    if key_data:
        # Try to get redirect URLs for each app
        try:
            # Get Happ redirect URL (encrypted if enabled)
            happ_url = await xui_api.get_subscription_link_for_key(key_data, app_type="happ", use_redirect=True)
            if happ_url:
                builder.button(text="📱 Happ", url=happ_url)
            else:
                # Fallback to callback if link fetch fails
                builder.button(text="📱 Happ", callback_data=f"happ_link_{key_id}")
        except Exception:
            # Fallback to callback if error occurs
            builder.button(text="📱 Happ", callback_data=f"happ_link_{key_id}")

        try:
            # Get v2RayTun redirect URL (always plain)
            v2raytun_url = await xui_api.get_subscription_link_for_key(key_data, app_type="v2raytun", use_redirect=True)
            if v2raytun_url:
                builder.button(text="🔷 v2RayTun", url=v2raytun_url)
            else:
                builder.button(text="🔷 v2RayTun", callback_data=f"v2raytun_link_{key_id}")
        except Exception:
            builder.button(text="🔷 v2RayTun", callback_data=f"v2raytun_link_{key_id}")

        try:
            # Get Streisand redirect URL (always plain)
            streisand_url = await xui_api.get_subscription_link_for_key(key_data, app_type="streisand", use_redirect=True)
            if streisand_url:
                builder.button(text="🔶 Streisand", url=streisand_url)
            else:
                builder.button(text="🔶 Streisand", callback_data=f"streisand_link_{key_id}")
        except Exception:
            builder.button(text="🔶 Streisand", callback_data=f"streisand_link_{key_id}")
    else:
        # If key data not found, use callback buttons as fallback
        builder.button(text="📱 Happ", callback_data=f"happ_link_{key_id}")
        builder.button(text="🔷 v2RayTun", callback_data=f"v2raytun_link_{key_id}")
        builder.button(text="🔶 Streisand", callback_data=f"streisand_link_{key_id}")

    # App installation links button
    builder.button(text="📲 Где установить приложения?", callback_data=f"show_app_install_links_{key_id}")
...
def create_app_install_links_keyboard(key_id: int) -> InlineKeyboardMarkup:
    """Create keyboard with app installation links for iOS and Android."""
    builder = InlineKeyboardBuilder()

    # iOS apps
    builder.button(text="📱 Happ (iOS)", url="https://apps.apple.com/ru/app/happ-proxy-utility-plus/id6746188973")
    builder.button(text="📱 v2RayTun (iOS)", url="https://apps.apple.com/ru/app/v2raytun/id6476628951")
    builder.button(text="📱 Streisand (iOS)", url="https://apps.apple.com/ru/app/streisand/id6450534064")

    # Android apps
    builder.button(text="🤖 Happ (Android)", url="https://play.google.com/store/apps/details?id=com.happproxy")
    builder.button(text="🤖 v2RayTun (Android)", url="https://play.google.com/store/apps/details?id=com.v2raytun.android")
...
def create_howto_vless_keyboard() -> InlineKeyboardMarkup:
    ...
    # Получаем URL-ссылки на Telegraph статьи из настроек (или используем mock-ссылки)
    android_url = get_setting("howto_android_url") or "https://telegra.ph/Instrukciya-Android-VLESS-01-01"
    ios_url = get_setting("howto_ios_url") or "https://telegra.ph/Instrukciya-iOS-VLESS-01-01"
    windows_url = get_setting("howto_windows_url") or "https://telegra.ph/Instrukciya-Windows-VLESS-01-01"
    linux_url = get_setting("howto_linux_url") or "https://telegra.ph/Instrukciya-Linux-VLESS-01-01"

    # Создаем кнопки со ссылками
    builder.button(text=(get_setting("btn_howto_android") or "📱 Android"), url=android_url)
    builder.button(text=(get_setting("btn_howto_ios") or "📱 iOS"), url=ios_url)
    builder.button(text=(get_setting("btn_howto_windows") or "💻 Windows"), url=windows_url)
    builder.button(text=(get_setting("btn_howto_linux") or "🐧 Linux"), url=linux_url)
    builder.button(text=(get_setting("btn_back_to_menu") or "⬅️ Назад в меню"), callback_data="back_to_main_menu")
    builder.adjust(2, 2, 1)
    return builder.as_markup()
