...
async def safe_edit_or_send(message: types.Message, text: str = None, reply_markup=None, photo_path: str = None, disable_web_page_preview: bool = False, **kwargs):
    """
    Универсальная функция для безопасного редактирования или отправки сообщения.
    Если сообщение содержит фото, удаляет его и отправляет новое.
    Поддерживает все параметры edit_text/send_message.
    """
    try:
        ...
        if photo_path:
            ...
            await message.edit_text(text, reply_markup=reply_markup, disable_web_page_preview=disable_web_page_preview, **kwargs)
    except TelegramBadRequest as e:
        # Если не получилось отредактировать (например, сообщение с фото, а мы пытаемся edit_text)
...
async def show_main_menu(message: types.Message, edit_message: bool = False):
    ...
    import os
    banner_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "images", "banner.png")
    ...
    if os.path.exists(banner_path):
        ...
        if edit_message:
            try:
                # При редактировании удаляем старое сообщение
                await message.delete()
            except Exception as e:
        ...
        try:
            # Используем chat.id для отправки нового сообщения
        ...
        # Если файл не существует, отправляем только текст (старое поведение)
        logger.warning(f"Файл баннера не найден: {banner_path}")
        if edit_message:
            try:
...
def get_user_router() -> Router:
    ...
    async def start_handler(message: types.Message, state: FSMContext, bot: Bot, command: CommandObject):
        ...
        if user_data and user_data.get('agreed_to_terms'):
            ...
            # Показываем главное меню с фото
        ...
        if os.path.exists(banner_path):
            try:
                ...
                logger.warning(f"Не удалось отправить приветственное фото: {e}")
                # Fallback: отправляем без фото
                await message.answer(
                    final_text,
                    reply_markup=keyboards.create_welcome_keyboard(
    ...
    async def profile_handler_callback(callback: types.CallbackQuery):
        ...
        await safe_edit_or_send(callback.message, final_text, reply_markup=keyboards.create_profile_keyboard())
    ...
    async def profile_info_handler(callback: types.CallbackQuery):
        ...
        await safe_edit_or_send(callback.message, final_text, reply_markup=builder.as_markup())
    ...
    async def topup_pay_yookassa(callback: types.CallbackQuery, state: FSMContext):
        ...
        if amount <= 0:
            await safe_edit_or_send(callback.message, "❌ Некорректная сумма пополнения. Повторите ввод.")
            await state.clear()
            return
        user_id = callback.from_user.id
    ...
    async def topup_pay_yoomoney(callback: types.CallbackQuery, state: FSMContext):
        ...
        if not ym_wallet:
            await safe_edit_or_send(callback.message, "❌ Оплата через ЮMoney временно недоступна.")
            await state.clear()
            return
        user_id = callback.from_user.id
    ...
    async def topup_pay_stars(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
        ...
        try:
            ...
            await safe_edit_or_send(callback.message, "❌ Не удалось создать счёт Stars. Попробуйте другой способ оплаты.")
            await state.clear()
            return

    @user_router.callback_query(TopUpProcess.waiting_for_topup_method, (F.data == "topup_pay_cryptobot") | (F.data == "topup_pay_heleket") | (F.data == "topup_pay_platega") | (F.data == "topup_pay_urlpay"))
    async def topup_pay_heleket_like(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            ...
            elif callback.data == "topup_pay_platega":
                pay_url = await _create_platega_payment_request(
                    user_id=user_id,
                    price=float(amount),
                    months=0,
                    host_name="",
            ...
            elif callback.data == "topup_pay_urlpay":
                pay_url = await _create_urlpay_payment_request(
                    user_id=user_id,
                    price=float(amount),
                    months=0,
                    host_name="",
            ...
            if pay_url:
                ...
                await safe_edit_or_send(callback.message, "❌ Не удалось создать счёт. Попробуйте другой способ оплаты.")
                await state.clear()
        except Exception as e:
    ...
    async def topup_pay_tonconnect(callback: types.CallbackQuery, state: FSMContext):
        ...
        if not wallet_address:
            await safe_edit_or_send(callback.message, "❌ Оплата через TON временно недоступна.")
            await state.clear()
            return
        ...
        if not usdt_rub_rate or not ton_usdt_rate:
            await safe_edit_or_send(callback.message, "❌ Не удалось получить курс TON. Попробуйте позже.")
            await state.clear()
            return
        ...
        try:
            ...
            await safe_edit_or_send(callback.message, "❌ Не удалось подготовить оплату TON Connect.")
            await state.clear()
    ...
    async def about_handler(callback: types.CallbackQuery):
        ...
        if support_bot_username:
            ...
            if support_user:
                ...
                await safe_edit_or_send(callback.message, "Контакты поддержки не настроены.", reply_markup=keyboards.create_back_to_menu_keyboard())
    ...
    async def support_external_handler(callback: types.CallbackQuery):
        ...
        support_user = get_setting("support_user")
        if not support_user:
            await safe_edit_or_send(callback.message, "Внешний контакт поддержки не настроен.", reply_markup=keyboards.create_back_to_menu_keyboard())
            return
        await safe_edit_or_send(callback.message, 
            "Для связи с поддержкой используйте кнопку ниже.",
            reply_markup=keyboards.create_support_keyboard(support_user)
    ...
    async def trial_period_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        hosts = get_all_hosts()
        if not hosts:
            await safe_edit_or_send(callback.message, "❌ В данный момент нет доступных серверов для создания пробного ключа.")
            return
    ...
    async def select_host_callback_handler(callback: types.CallbackQuery):
        ...
        if action == "new":
            ...
            if not plans:
                await safe_edit_or_send(callback.message, f"❌ Для сервера \"{host_name}\" не настроены тарифы.")
                return
            await safe_edit_or_send(callback.message, 
    ...
    async def process_trial_key_creation(message: types.Message, host_name: str):
        user_id = message.chat.id
        await safe_edit_or_send(message, f"Отлично! Создаю для вас бесплатный ключ на {get_setting('trial_duration_days')} дня на сервере \"{host_name}\"...")
        ...
        try:
            ...
            if not result:
                await safe_edit_or_send(message, "❌ Не удалось создать пробный ключ. Ошибка на сервере.")
                return
            ...
            try:
                await safe_edit_or_send(message, text=final_text, reply_markup=await keyboards.create_key_info_keyboard(new_key_id), disable_web_page_preview=True)
            except TelegramBadRequest:
                ...
                try:
                    ...
                    pass
                await message.answer(text=final_text, reply_markup=await keyboards.create_key_info_keyboard(new_key_id))

        except Exception as e:
            logger.error(f"Ошибка создания пробного ключа для пользователя {user_id} на хосте {host_name}: {e}", exc_info=True)
            await safe_edit_or_send(message, "❌ Произошла ошибка при создании пробного ключа.")
    ...
    async def show_key_handler(callback: types.CallbackQuery):
        ...
        if not key_data or key_data['user_id'] != user_id:
            await safe_edit_or_send(callback.message, "❌ Ошибка: ключ не найден.")
            return

        try:
            import os
            banner_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "images", "banner.png")
            caption_text = "Выбери приложение ниже и активируй подписку"

            await safe_edit_or_send(
                callback.message,
                text=caption_text,
                reply_markup=await keyboards.create_key_info_keyboard(key_id_to_show),
                photo_path=banner_path
            )
        except Exception as e:
            logger.error(f"Ошибка показа ключа {key_id_to_show}: {e}")
            await safe_edit_or_send(callback.message, "❌ Произошла ошибка при получении данных ключа.")
        ...
        try:
            ...
            try:
                ...
                if details and details.get('connection_string'):
                    ...
                    await safe_edit_or_send(callback.message,
                        text=final_text,
                        reply_markup=await keyboards.create_key_info_keyboard(key_id)
                    )
                else:
                    await safe_edit_or_send(callback.message, 
    ...
    @user_router.callback_query(F.data.startswith("happ_link_"))
    @registration_required
    async def happ_link_handler(callback: types.CallbackQuery):
        """Handler for Happ app button - shows encrypted link if enabled, otherwise plain."""
        await callback.answer()
        try:
            key_id = int(callback.data.split("_")[2])
            ...
            await safe_edit_or_send(
                callback.message,
                text="Выбери приложение ниже и активируй подписку",
                reply_markup=await keyboards.create_key_info_keyboard(key_id),
                photo_path=banner_path
            )

        except Exception as e:
            logger.error(f"Ошибка получения ссылки Happ для ключа: {e}", exc_info=True)
            await callback.answer("❌ Произошла ошибка.", show_alert=True)

    @user_router.callback_query(F.data.startswith("v2raytun_link_"))
    @registration_required
    async def v2raytun_link_handler(callback: types.CallbackQuery):
        """Handler for v2RayTun app button - always shows plain link."""
        await callback.answer()
        try:
            key_id = int(callback.data.split("_")[2])
            ...
            logger.error(f"Ошибка получения ссылки v2RayTun для ключа: {e}", exc_info=True)
            await callback.answer("❌ Произошла ошибка.", show_alert=True)

    @user_router.callback_query(F.data.startswith("streisand_link_"))
    @registration_required
    async def streisand_link_handler(callback: types.CallbackQuery):
        """Handler for Streisand app button - always shows plain link."""
        await callback.answer()
        try:
            key_id = int(callback.data.split("_")[2])
            ...
            logger.error(f"Ошибка получения ссылки Streisand для ключа: {e}", exc_info=True)
            await callback.answer("❌ Произошла ошибка.", show_alert=True)

    @user_router.callback_query(F.data.startswith("show_app_install_links_"))
    @registration_required
    async def show_app_install_links_handler(callback: types.CallbackQuery):
        """Handler for showing app installation links."""
        await callback.answer()
        try:
            key_id = int(callback.data.split("_")[4])

            text = (
                "<b>📲 Где установить приложения?</b>\n\n"
                "<b>iOS (App Store):</b>\n"
                "• Happ - универсальное приложение\n"
                "• v2RayTun - альтернативный клиент\n"
                "• Streisand - ещё один клиент\n\n"
                "<b>Android (Google Play):</b>\n"
                "• Happ - универсальное приложение\n"
                "• v2RayTun - альтернативный клиент\n\n"
                "<i>Нажмите на кнопку ниже, чтобы перейти к установке нужного приложения</i>"
            )

            await safe_edit_or_send(
                callback.message,
                text,
                reply_markup=keyboards.create_app_install_links_keyboard(key_id)
            )

        except Exception as e:
            logger.error(f"Ошибка показа ссылок на приложения: {e}", exc_info=True)
            await callback.answer("❌ Произошла ошибка.", show_alert=True)
    ...
    # Обработчики для инструкций удалены, так как теперь используются прямые ссылки на Telegraph
    ...
    async def buy_new_key_handler(callback: types.CallbackQuery):
        ...
        hosts = get_all_hosts()
        if not hosts:
            await safe_edit_or_send(callback.message, "❌ В данный момент нет доступных серверов для покупки.")
            return
    ...
    async def extend_key_handler(callback: types.CallbackQuery):
        ...
        try:
            ...
            await safe_edit_or_send(callback.message, "❌ Произошла ошибка. Неверный формат ключа.")
            return

        key_data = get_key_by_id(key_id)

        if not key_data or key_data['user_id'] != callback.from_user.id:
            await safe_edit_or_send(callback.message, "❌ Ошибка: Ключ не найден или не принадлежит вам.")
            return
        
        host_name = key_data.get('host_name')
        if not host_name:
            await safe_edit_or_send(callback.message, "❌ Ошибка: У этого ключа не указан сервер. Обратитесь в поддержку.")
            return
    ...
    async def plan_selection_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        await state.update_data(
            action=action, key_id=key_id, plan_id=plan_id, host_name=host_name, customer_email=None
        )

        # Skip email verification and go directly to payment options
        await show_payment_options(callback.message, state)
        logger.info(f"Пользователь {callback.from_user.id}: Переход к выбору способа оплаты")
    ...
    async def show_payment_options(message: types.Message, state: FSMContext):
        ...
        if not plan:
            try:
                await safe_edit_or_send(message, "❌ Ошибка: Тариф не найден.")
            except TelegramBadRequest:
    ...
    async def create_yoomoney_payment_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        plan = get_plan_by_id(data.get('plan_id'))
        if not plan:
            await safe_edit_or_send(callback.message, "❌ Произошла ошибка при выборе тарифа.")
            await state.clear()
            return
    ...
    async def create_platega_payment_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        if pay_url:
            ...
            await safe_edit_or_send(callback.message, "❌ Не удалось создать счет Platega. Попробуйте другой способ оплаты.")

    @user_router.callback_query(PaymentProcess.waiting_for_payment_method, F.data == "pay_urlpay")
    async def create_urlpay_payment_handler(callback: types.CallbackQuery, state: FSMContext):
        await callback.answer("Создаю ссылку на оплату...")
        ...
        if pay_url:
            ...
            await safe_edit_or_send(callback.message, "❌ Не удалось создать счет URLPay. Попробуйте другой способ оплаты.")
    ...
    async def create_cryptobot_invoice_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        if not cryptobot_token:
            ...
            await safe_edit_or_send(callback.message, "❌ Оплата криптовалютой временно недоступна. (Администратор не указал токен).")
            await state.clear()
            return
        ...
        if pay_url:
            ...
            await safe_edit_or_send(callback.message, "❌ Не удалось создать счет CryptoBot. Попробуйте другой способ оплаты.")
    ...
    async def pay_with_main_balance_handler(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
        ...
        plan = get_plan_by_id(data.get('plan_id'))
        if not plan:
            await safe_edit_or_send(callback.message, "❌ Ошибка: Тариф не найден.")
            await state.clear()
            return
        months = int(plan['months'])
...
async def _create_platega_payment_request(
    user_id: int,
    price: float,
    months: int,
    host_name: str,
    ...
    """Создать счёт через Platega и вернуть ссылку на оплату.
    ...
    try:
        ...
        if not merchant_id or not api_key:
            logger.error("Platega: отсутствуют merchant_id/api_key в настройках.")
            return None

        # Генерируем уникальный ID транзакции
        transaction_id = str(uuid.uuid4())
        logger.info(f"[PLATEGA] Создаём транзакцию с ID: {transaction_id} для пользователя {user_id}")
        ...
        try:
            create_pending_transaction(transaction_id, user_id, float(price), metadata)
            logger.info(f"[PLATEGA] ✅ Pending транзакция создана: {transaction_id}")
        except Exception as e:
            logger.error(f"[PLATEGA] ❌ Не удалось создать ожидающую транзакцию: {e}", exc_info=True)
        ...
        # Формируем payload для Platega (используем поле payload для передачи метаданных)
        payload_str = json.dumps(metadata, ensure_ascii=False, separators=(",", ":"))

        # Тело запроса к Platega API
        request_body = {
            "paymentMethod": 2,  # По умолчанию СБП/QR (можно сделать настраиваемым)
            "id": transaction_id,
            "paymentDetails": {
                "amount": float(price),
                "currency": "RUB"
            },
            "description": f"Подписка на {months} мес." if months > 0 else "Пополнение баланса",
            "payload": payload_str
        }
        ...
        # Отправляем запрос к Platega API
        endpoint = "https://app.platega.io/transaction/process"

        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(endpoint, json=request_body, headers=headers, timeout=15) as resp:
                    text = await resp.text()
                    if resp.status not in (200, 201):
                        logger.error(f"Platega: не удалось создать транзакцию (HTTP {resp.status}): {text}")
                        return None
                    try:
                        data_json = await resp.json()
                    except Exception:
                        logger.warning(f"Platega: неожиданный ответ (не JSON): {text}")
                        return None

                    # Получаем ссылку на оплату из ответа
                    pay_url = data_json.get("redirect")
                    if not pay_url:
                        logger.error(f"Platega: не найдено поле redirect в ответе: {data_json}")
                        return None

                    logger.info(f"Platega: создана транзакция {transaction_id} для пользователя {user_id}")
                    return str(pay_url)
            except Exception as e:
                logger.error(f"Platega: ошибка HTTP при создании транзакции: {e}", exc_info=True)
                return None
    except Exception as e:
        logger.error(f"Platega: общая ошибка при создании транзакции: {e}", exc_info=True)
        return None

async def _create_urlpay_payment_request(
    user_id: int,
    price: float,
    months: int,
    host_name: str,
    ...
    """Создать счёт через URLPay и вернуть ссылку на оплату.

    Возвращает URL на оплату или None при ошибке.
    """
    try:
        shop_id = get_setting("urlpay_shop_id")
        api_key = get_setting("urlpay_api_key")
        secret_key = get_setting("urlpay_secret_key")

        if not shop_id or not api_key or not secret_key:
            logger.error("URLPay: отсутствуют shop_id/api_key/secret_key в настройках.")
            return None

        # Генерируем уникальный UUID для заказа
        order_uuid = str(uuid.uuid4())
        logger.info(f"[URLPAY] Создаём транзакцию с UUID: {order_uuid} для пользователя {user_id}")

        # Метаданные для вебхука
        metadata = {
            "payment_id": order_uuid,
            "user_id": user_id,
            "months": months,
            "price": float(price),
            "action": state_data.get("action"),
        ...
        # Сохраняем pending транзакцию
        try:
            create_pending_transaction(order_uuid, user_id, float(price), metadata)
            logger.info(f"[URLPAY] ✅ Pending транзакция создана: {order_uuid}")
        except Exception as e:
            logger.error(f"[URLPAY] ❌ Не удалось создать ожидающую транзакцию: {e}", exc_info=True)

        # Формируем подпись: SHA1(currency + amount + shopId + secret_key)
        import hashlib
        currency = "rub"
        amount_str = f"{price:.2f}"
        sign_string = f"{currency}{amount_str}{shop_id}{secret_key}"
        sign = hashlib.sha1(sign_string.encode('utf-8')).hexdigest()

        # Описание платежа
        description = f"Подписка на {months} мес." if months > 0 else "Пополнение баланса"

        # Формируем items для чека (обязательно по API)
        items = [
            {
                "description": description,
                "quantity": 1,
                "price": float(price),
                "vat_code": 0,  # Без НДС
                "payment_subject": 4,  # Услуга
                "payment_mode": 4,  # Полный расчет
            }
        ]

        # Тело запроса к URLPay API
        request_body = {
            "currency": currency,
            "amount": amount_str,
            "uuid": order_uuid,
        ...
        # Отправляем запрос к URLPay API
        endpoint = "https://urlpay.io/api/v2/payments"
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(endpoint, json=request_body, headers=headers, timeout=15) as resp:
                    text = await resp.text()
                    if resp.status not in (200, 201):
                        logger.error(f"URLPay: не удалось создать транзакцию (HTTP {resp.status}): {text}")
                        return None
                    try:
                        data_json = await resp.json()
                    except Exception:
                        logger.warning(f"URLPay: неожиданный ответ (не JSON): {text}")
                        return None

                    # Проверяем успех
                    if not data_json.get("success"):
                        logger.error(f"URLPay: ошибка создания платежа: {data_json}")
                        return None

                    # Получаем ссылку на оплату из ответа
                    pay_url = data_json.get("paymentUrl")
                    if not pay_url:
                        logger.error(f"URLPay: не найдено поле paymentUrl в ответе: {data_json}")
                        return None

                    import time
                    logger.info(f"[URLPAY] ✅ Создана транзакция {order_uuid} для пользователя {user_id} в {time.strftime('%H:%M:%S')}")
                    return str(pay_url)
            except Exception as e:
                logger.error(f"URLPay: ошибка HTTP при создании транзакции: {e}", exc_info=True)
                return None
    except Exception as e:
        logger.error(f"URLPay: общая ошибка при создании транзакции: {e}", exc_info=True)
        return None
    ...
    try:
        ...
        if action == "new":
            ...
            if not existing_key or not existing_key.get('key_email'):
                await safe_edit_or_send(processing_message, "❌ Не удалось найти ключ для продления.")
                return
        ...
        if not result:
            await safe_edit_or_send(processing_message, "❌ Не удалось создать/обновить ключ в панели.")
            return
        ...
        try:
            await safe_edit_or_send(processing_message, "❌ Ошибка при выдаче ключа.")
        except Exception:
            try:
