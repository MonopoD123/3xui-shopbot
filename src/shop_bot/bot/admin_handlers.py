...
def get_admin_router() -> Router:
    ...
    async def _send_monitor_view(message: types.Message, edit_message: bool = False):
        ...
        if edit_message:
            try:
                await safe_edit_or_send(message, full_text, reply_markup=keyboard)
            except Exception:
                pass
        else:
    ...
    async def admin_speedtest_run(callback: types.CallbackQuery):
        ...
        if wait_msg:
            try:
                await safe_edit_or_send(wait_msg, text_res)
            except Exception:
    ...
    async def admin_backup_db(callback: types.CallbackQuery):
        ...
        if not zip_path:
            if wait:
                await safe_edit_or_send(wait, "❌ Не удалось создать бэкап БД")
            else:
        ...
        if wait:
            try:
                await safe_edit_or_send(wait, txt)
            except Exception:
    ...
    async def admin_restore_db_prompt(callback: types.CallbackQuery, state: FSMContext):
        ...
        try:
            await safe_edit_or_send(callback.message, text, reply_markup=kb.as_markup())
        except Exception:
    ...
    async def admin_speedtest_autoinstall(callback: types.CallbackQuery):
        ...
        if wait:
            try:
                await safe_edit_or_send(wait, text)
            except Exception:
                await callback.message.answer(text)
        else:
            await callback.message.answer(text)
    ...
    async def _send_promo_summary(message_or_msg, state: FSMContext, edit: bool = False):
        ...
        if edit:
            try:
                await safe_edit_or_send(message_or_msg, text, reply_markup=kb)
            except Exception:
    ...
    async def confirm_broadcast_handler(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
        await safe_edit_or_send(callback.message, "⏳ Начинаю рассылку... Это может занять некоторое время.")
