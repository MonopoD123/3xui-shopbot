...
ALL_SETTINGS_KEYS = [
    ...
    # Telegraph URLs for instructions
    "howto_android_url", "howto_ios_url", "howto_windows_url", "howto_linux_url",
    # Button texts
    ...
    "heleket_merchant_id", "heleket_api_key", "domain", "shortio_api_key", "shortio_domain", "referral_percentage",
    ...
    # URLPay
    "urlpay_shop_id", "urlpay_api_key", "urlpay_secret_key",
    # Happ encryption
    "happ_encrypt_subscription",
]
...
def create_webhook_app(bot_controller_instance):
    ...
    def settings_page():
        if request.method == 'POST':
            ...
            checkbox_keys = ['force_subscription', 'sbp_enabled', 'trial_enabled', 'enable_referrals', 'enable_fixed_referral_bonus', 'stars_enabled', 'yoomoney_enabled', 'monitoring_enabled', 'happ_encrypt_subscription']
    ...
    def platega_webhook_handler():
        ...
        Platega отправляет callback с заголовками X-MerchantId и X-Secret для аутентификации.
        Тело запроса содержит информацию о транзакции в формате JSON.
        """
        try:
            data = request.json
            logger.info(f"[PLATEGA WEBHOOK] Получен вебхук: {data}")
            logger.info(f"[PLATEGA WEBHOOK] Заголовки: X-MerchantId={request.headers.get('X-MerchantId')}, X-Secret={'***' if request.headers.get('X-Secret') else 'None'}")
            ...
            merchant_id_setting = get_setting("platega_merchant_id")
            api_key_setting = get_setting("platega_api_key")

            if not merchant_id_setting or not api_key_setting:
                logger.error("Platega webhook: настройки merchant_id/api_key не найдены")
                return 'Error', 500

            # Проверяем аутентификацию
            if not merchant_id_header or not secret_header:
                logger.warning("Platega webhook: отсутствуют заголовки аутентификации")
                return 'Unauthorized', 401

            if not compare_digest(merchant_id_header, merchant_id_setting) or not compare_digest(secret_header, api_key_setting):
                logger.warning("Platega webhook: неверные учетные данные")
                return 'Forbidden', 403

            # Проверяем статус платежа
            status = data.get('status')
            logger.info(f"[PLATEGA WEBHOOK] Статус платежа: {status}")

            if status == 'CONFIRMED':
                # Успешная оплата
                transaction_id = data.get('id')
                if not transaction_id:
                    logger.error("Platega webhook: отсутствует ID транзакции")
                    return 'Error', 400

                logger.info(f"[PLATEGA WEBHOOK] Ищем pending транзакцию с ID: {transaction_id}")

                # Завершаем pending транзакцию и получаем метаданные
                amount_rub = float(data.get('amount', 0)) if data.get('amount') else None
                currency = data.get('currency', 'RUB')

                metadata = database.find_and_complete_pending_transaction(
                    payment_id=transaction_id,
                    amount_rub=amount_rub,
                    payment_method="Platega",
                    currency_name=currency,
                    amount_currency=amount_rub,
                )

                if not metadata:
                    logger.error(f"[PLATEGA WEBHOOK] ❌ НЕ НАЙДЕНА pending транзакция для ID {transaction_id}! Проверьте таблицу transactions.")
                    return 'OK', 200  # Возвращаем OK, чтобы Platega не повторял запрос

                logger.info(f"[PLATEGA WEBHOOK] ✅ Найдена транзакция! Метаданные: {metadata}")
                ...
                if bot and loop and loop.is_running():
                    asyncio.run_coroutine_threadsafe(payment_processor(bot, metadata), loop)
                    logger.info(f"[PLATEGA WEBHOOK] ✅ Успешно обработан платеж {transaction_id}, отправлено в process_successful_payment")
                else:
                    logger.error("[PLATEGA WEBHOOK] ❌ Бот или цикл событий недоступны!")

            elif status == 'CANCELED':
                # Неуспешная оплата
                logger.info(f"Platega webhook: платеж отменен для транзакции {data.get('id')}")

            return 'OK', 200

        except Exception as e:
            logger.error(f"[PLATEGA WEBHOOK] ❌ Ошибка в обработчике: {e}", exc_info=True)
            return 'Error', 500

    @csrf.exempt
    @flask_app.route('/urlpay-webhook', methods=['POST'])
    def urlpay_webhook_handler():
        """Обработчик webhook от URLPay.

        URLPay отправляет callback с данными о платеже в формате JSON.
        """
        import time
        webhook_start_time = time.time()
        try:
            data = request.json
            logger.info(f"[URLPAY WEBHOOK] ⏱️ Получен вебхук в {time.strftime('%H:%M:%S')}: {data}")

            # Извлекаем данные из webhook
            payment_id = data.get('id')
            amount = data.get('amount')
            currency = data.get('currency', 'RUB')
            uuid_order = data.get('uuid')
            payment_status = data.get('payment_status')

            logger.info(f"[URLPAY WEBHOOK] Статус платежа: {payment_status}, UUID: {uuid_order}")

            # Проверяем статус платежа
            if payment_status == 'success':
                # Успешная оплата
                if not uuid_order:
                    logger.error("URLPay webhook: отсутствует UUID заказа")
                    return 'Error', 400

                logger.info(f"[URLPAY WEBHOOK] Ищем pending транзакцию с UUID: {uuid_order}")

                # Завершаем pending транзакцию и получаем метаданные
                amount_rub = float(amount) if amount else None

                metadata = database.find_and_complete_pending_transaction(
                    payment_id=uuid_order,
                    amount_rub=amount_rub,
                ...
                if not metadata:
                    logger.error(f"[URLPAY WEBHOOK] ❌ НЕ НАЙДЕНА pending транзакция для UUID {uuid_order}! Проверьте таблицу transactions.")
                    return 'OK', 200  # Возвращаем OK, чтобы URLPay не повторял запрос

                logger.info(f"[URLPAY WEBHOOK] ✅ Найдена транзакция! Метаданные: {metadata}")
                ...
                if bot and loop and loop.is_running():
                    ...
                    logger.info(f"[URLPAY WEBHOOK] ✅ Успешно обработан платеж {uuid_order}, отправлено в process_successful_payment (обработка заняла {webhook_duration:.2f}с)")
                else:
                    logger.error("[URLPAY WEBHOOK] ❌ Бот или цикл событий недоступны!")

            elif payment_status == 'cancel':
                # Неуспешная оплата
                logger.info(f"URLPay webhook: платеж отменен для заказа {uuid_order}")

            webhook_duration = time.time() - webhook_start_time
            logger.info(f"[URLPAY WEBHOOK] ⏱️ Возвращаем OK (общее время: {webhook_duration:.2f}с)")
            return 'OK', 200

        except Exception as e:
            logger.error(f"[URLPAY WEBHOOK] ❌ Ошибка в обработчике: {e}", exc_info=True)
            return 'Error', 500
