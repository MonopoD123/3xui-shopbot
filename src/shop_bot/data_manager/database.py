    ...
    try:
        with sqlite3.connect(DB_FILE) as conn:
            ...
            default_settings = {
                ...
                "howto_android_url": None,
                "howto_ios_url": None,
                "howto_windows_url": None,
                "howto_linux_url": None,
                ...
                "domain": None,
                "shortio_api_key": None,  # short.io API key for URL shortening
                "shortio_domain": None,  # short.io domain for shortened links
                ...
                # Happ link encryption (RSA-4096)
                "happ_encrypt_subscription": "true",  # Включить шифрование ссылок подписки в формате happ://crypt3/
            }
            run_migration()
...
def find_and_complete_pending_transaction(
    ...
    try:
        with sqlite3.connect(DB_FILE) as conn:
            ...
            logger.info(f"[DB] Ищем pending транзакцию: payment_id={payment_id}")
            cursor.execute("SELECT * FROM transactions WHERE payment_id = ? AND status = 'pending'", (payment_id,))
            transaction = cursor.fetchone()
            if not transaction:
                logger.warning(f"[DB] ❌ Ожидающая транзакция НЕ НАЙДЕНА для payment_id={payment_id}")
                # Дополнительная диагностика: проверим все pending транзакции
                cursor.execute("SELECT payment_id, user_id, amount_rub, created_at FROM transactions WHERE status = 'pending' ORDER BY created_at DESC LIMIT 5")
                recent_pending = cursor.fetchall()
                if recent_pending:
                    logger.info(f"[DB] Последние 5 pending транзакций:")
                    for row in recent_pending:
                        logger.info(f"  - payment_id={row['payment_id']}, user_id={row['user_id']}, amount={row['amount_rub']}, created={row['created_at']}")
                else:
                    logger.info(f"[DB] В таблице нет pending транзакций")
                return None

            logger.info(f"[DB] ✅ Найдена pending транзакция: user_id={transaction['user_id']}, amount={transaction['amount_rub']}")
            ...
            conn.commit()
            logger.info(f"[DB] ✅ Транзакция обновлена на статус 'paid'")
            ...
            try:
                ...
                logger.info(f"[DB] ✅ Метаданные извлечены: {md}")
            except Exception as e:
                logger.error(f"[DB] ❌ Ошибка парсинга метаданных: {e}")
                md = {}
            return md
    except sqlite3.Error as e:
        logging.error(f"[DB] ❌ Не удалось завершить ожидающую транзакцию {payment_id}: {e}")
        return None
